...
public class ExceptionTranslator {
    ...
    public ResponseEntity<ResultBean> processCustomError(BussinessException ex) {
        logger.error(ex.getMessage(), ex);
//        return new ResponseEntity<>(ResultBean.error(ex.getCode(),ex.getMessage()), HttpStatus.BAD_REQUEST);
...
/*    private ResponseEntity<ResultBean> processException(HttpStatus httpStatus){
        return new ResponseEntity<>(ResultBean.error(httpStatus.value(), httpStatus.getReasonPhrase()),httpStatus);
    }
